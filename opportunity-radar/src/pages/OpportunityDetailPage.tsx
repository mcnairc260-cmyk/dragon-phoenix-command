...
function Section({ label, title, children }: { label: string; title: string; children: React.ReactNode }) {
  return (
    <section className="border-t border-line py-6">
      <p className="mb-1 text-[11px] font-semibold tracking-[0.1em] text-muted uppercase">{label}</p>
      <h2 className="mb-3 font-display text-lg font-bold text-ash">{title}</h2>
      {children}
    </section>
  );
}
    ...
    <ol className="space-y-2">
      {items.map((item, i) => (
        <li key={i} className="flex gap-3 text-sm leading-relaxed text-body">
          <span className="text-xs font-semibold text-gold">{String(i + 1).padStart(2, '0')}</span>
          <span>{item}</span>
        </li>
      ))}
    </ol>
...
function DetailSkeleton() {
  return (
    <div className="animate-pulse space-y-4" role="status" aria-label="Loading opportunity">
      ...
      <div className="h-8 w-2/3 rounded bg-surface3" />
      <div className="h-24 w-full rounded-xl bg-surface3" />
      <div className="h-40 w-full rounded-xl bg-surface3" />
...
function DataProvenanceNote() {
  return (
    <div className="rounded-xl border border-ember/30 bg-ember/8 p-4 text-xs leading-relaxed text-body">
      <p className="mb-1 text-[10px] font-semibold tracking-[0.08em] text-gold uppercase">
...
export default function OpportunityDetailPage() {
  ...
  return (
    <article>
      <nav aria-label="Breadcrumb" className="mb-4 text-[10px] font-medium text-muted">
      ...
      {/* Header — title column beside the score gauge so the gauge's height
          doesn't open a gap above the badge row. */}
      <header className="mb-6 flex flex-col-reverse gap-5 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0 flex-1">
          <p className="mb-2 flex flex-wrap items-center gap-2 text-[10px] font-medium tracking-[0.14em] text-gold uppercase">
            {o.category}
            <span className="text-muted normal-case">
              · {o.opportunityType} · {o.locationMode}
            </span>
            <DemoBadge />
          </p>
          <h1 className="max-w-2xl font-display text-2xl leading-tight font-bold text-ash sm:text-3xl">
            {o.title}
          </h1>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            <button
              type="button"
              onClick={handleShare}
              className="inline-flex items-center gap-2 rounded-lg border border-line-strong px-4 py-2 text-sm font-bold text-ash hover:border-gold hover:text-gold"
            >
            ...
            <button
              ...
              className="inline-flex cursor-not-allowed items-center gap-2 rounded-lg border border-line px-4 py-2 text-sm font-bold text-muted"
            >
              <Icon name="document" /> Export report (Pro)
            </button>
          </div>
      ...
      <Section label="Timing" title="Why now?">
        ...
        <h3 className="mb-2 text-[10px] font-semibold tracking-[0.08em] text-gold uppercase">
          Key market signals <span className="text-muted normal-case">(AI-estimated, demo)</span>
        </h3>
        <ul className="space-y-3">
          {o.signals.map((signal) => (
            <li key={signal.headline} className="rounded-xl border border-line bg-surface p-4">
              <div className="mb-1.5 flex flex-wrap items-center justify-between gap-2">
                <span className="text-[10px] font-semibold tracking-[0.08em] text-gold uppercase">
                  {signal.category}
                </span>
      ...
      <Section label="Execution" title="Strategic action plan">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          <div className="rounded-xl border border-line bg-surface p-4">
            <h3 className="mb-3 text-[10px] font-semibold tracking-[0.08em] text-gold uppercase">
              First 7 days
            </h3>
