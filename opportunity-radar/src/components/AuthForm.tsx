...
export function AuthForm({ mode }: { mode: 'login' | 'signup' }) {
  ...
  return (
    <div className="mx-auto max-w-sm px-4 py-16">
      ...
      {!auth.isConfigured() && (
        <p className="mb-5 rounded-lg border border-ember/30 bg-ember/8 p-3 text-xs leading-relaxed text-body">
          <strong className="text-gold">Demo build:</strong> accounts aren't live yet. This screen
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-4">
        <label className="block">
          ...
          <input
            ...
            autoComplete="email"
            required
            className="w-full rounded-lg border border-line bg-surface px-3 py-2.5 text-sm text-ash placeholder:text-faint focus:border-gold focus:outline-none"
        ...
        <button
          type="submit"
          disabled={submitting}
          className="w-full rounded-lg bg-gold px-4 py-2.5 text-sm font-bold text-void transition-opacity hover:opacity-90 disabled:opacity-50"
        >
