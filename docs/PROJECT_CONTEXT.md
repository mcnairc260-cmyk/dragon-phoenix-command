...
## 19. BREAKPOINT — 3D pool game, Phase 1 vertical slice (2026-08-29)

A founder-directed session built **BREAKPOINT**, a physics-first 3D pool game, as a self-contained sub-application in `games/breakpoint/` — branch `claude/breakpoint-phase-1-pool-1sjmmi`. Full engineering record: `games/breakpoint/README.md`.

**Audit finding that shapes everything else.** The directive described BREAKPOINT as "a specification and partial implementation" and named an existing `PhysicsRegressionSuite`. **Neither exists in this repository.** `git log --all`, an added-files sweep across every branch, and a full-tree content search all come back empty: there is no BREAKPOINT document, no partial code, nothing in history. Phase 1 was therefore built from scratch. Nothing was deleted or rewritten because there was nothing to preserve. **A successor should not go looking for a prior BREAKPOINT spec — it is not in this repo, and if the founder has one it lives somewhere else and has not been shared.**

**What it is.** One regulation 9-foot table, a full eight-ball rack, and the complete shot loop — aim, set spin, set power, strike, watch, shoot again — on a deterministic 120 Hz simulation, rendered in three.js. No rules, no multiplayer, no progression: the core shot experience is the whole product at this phase, as the directive required.

**Decisions made (flag-worthy):**

1. **Third framework exception.** Vite + TypeScript + Vitest + three.js, scoped to `games/breakpoint/` only. This follows the pattern established by `opportunity-radar/` (§16) and `games/emberloop/` (§18): a sub-app may carry its own toolchain when the founder has approved that app; the root site stays vanilla and zero-dependency. `index.html`, `api/`, `vercel.json` and the root `package.json` are untouched. The founder's task prompt commissioned a 3D game with premium rendering, which is the recorded approval — but note this is an **inference from the brief**, not an explicit "you may add three.js", and it is the third such precedent rather than a new rule.
2. **Palette follows the Brand Bible**, consistent with the founder's 2026-08-20 ruling for EMBERLOOP (§18). Void Black, Carbon, Ember Orange, Rebirth Gold, Signal Cyan, Ghost White, Steel, all in `src/config/brand.ts`. The site's own palette is untouched and §8.1 remains open. Two documented deviations in that same file, on the same "functional, not brand" reasoning EMBERLOOP used: (a) a deep-teal **cloth** and near-black woods, because a table has to read as a table; (b) the **standard fifteen ball colours**, because telling fifteen balls apart at a glance is a hard gameplay requirement that three accents cannot meet.
3. **Zero external assets.** Ball numbers, stripes, the cue-ball spot and the cloth weave are drawn to canvases at boot; the environment map for the balls' reflections is a small procedural scene; all audio is synthesised. Nothing to license, no CDN dependency.
4. **Not deployed.** No Vercel project, nothing published, no money or credits spent. Deploying it is Tier 3 and would follow the EMBERLOOP precedent (a separate Vercel project with Root Directory `games/breakpoint`).
5. **`window.BREAKPOINT` is always exposed**, not gated behind `?debug=1` as EMBERLOOP's bridge is. It carries only game state — no secrets, no personal data — and it is what the automated browser verification drives. If the game is ever deployed publicly this is worth revisiting, but it exposes nothing sensitive today.

**The physics, and why it is not a black box.** Standard pocket-billiards model (Alciatore's technical proofs; Marlow) in SI units, deterministic, fixed 1/120 s timestep, with continuous collision detection *inside* each step. All of the required behaviours are real rather than cosmetic: angular velocity is integrated and drives the trajectory, the cushion contact point sits above the ball's centre so rails add topspin and English changes the rebound angle, ball–ball friction produces throw, and pocket rejection happens because a ball genuinely clipped a jaw circle. Two derived results are asserted by tests rather than assumed — a tip 2/5 of a radius high produces exact natural roll, and a stun cut sends the balls off at 90°.

**One design decision inside the integrator is worth recording**, because it is subtle and a successor could easily "simplify" it back into a bug. A contact can be geometrically touching yet not approaching once the contact point's own motion is counted (a ball with heavy draw held against a cushion is the normal case). Such a contact produces no impulse, and if it stays in the candidate set it is re-detected at t = 0 for the rest of the step, so the sub-step loop spins without advancing time and **the balls freeze mid-table with energy that never decays**. That was a real bug in this session, found because a break refused to settle. The fix is the `inert` set in `PhysicsWorld.step()` (retire a zero-impulse contact for the remainder of the step) plus a position-only `depenetrate()` pass. Removing either brings the freeze back.

**Verification performed** (Protocol §7.1 — exercised, not eyeballed):
- **43 Vitest tests**, all passing, covering the 18 regression scenarios the directive named plus rack, shot-loop, shot-record and aiming-line coverage. The physics tests assert conservation laws, signs and documented billiards facts — no expected value was copied out of a previous run.
- `tsc -b` clean, production build clean, ESLint clean (no pre-existing exceptions; the config is new).
- A **Playwright run against the real production build** in Chromium, driving the actual UI: mouse-drag aiming changed the aim angle, a drag on the spin widget set the tip to "draw + right", a pull-back gesture on the cue pad loaded 91% power and released into a break, controls were confirmed locked mid-shot, every ball reached rest with all values finite, a shot record was produced with the expected contents, and a second shot was played immediately afterwards. Screenshots were reviewed and **four real rendering bugs were found and fixed that way**: the extruded bed faced downwards so the table rendered as nothing at all; pocket liner tori stood proud of the cloth and read as croquet hoops; a continuous rail frame roofed over every pocket mouth; and an oversized contact-shadow quad painted over the lower half of the cue ball.

**Known limitations (honest list):** balls never leave the cloth, so there are no jumps, masse or curve from an elevated cue — vertical velocity is projected out at cushion contact, which is dissipative and cannot create energy, but a genuinely airborne ball is not modelled. Cushion restitution is a constant where a real cushion is less elastic at speed. The rack is fixed with no seeded jitter, so every identical break is identical — a determinism feature now, but it will need randomising once there are rules. The automated playtest runs under SwiftShader software rendering, so it proves correctness, not frame rate; the 60 FPS target needs a real device. No PWA/service worker.
...
## 20. Interviews — AI-moderated research conversations (2026-09-16)

A founder-directed session ("Build a Perspective.Ai clone that is exactly like it") built **Interviews**, an AI-moderated one-on-one research tool, as `interviews/` + `api/interviews.js` + `api/interview-ai.js` on branch `claude/perspective-ai-clone-61avgl`. Full engineering record: `interviews/README.md`.

**How the brief was read.** "Exactly like it" was taken as *functional equivalence*: the same product shape as commercial AI-interview tools (describe a goal → AI drafts a discussion guide → shareable link → AI moderator interviews each person with follow-ups, text or voice → per-conversation summary → cross-conversation themes with quotes and counts → ask-your-data chat → export). It was **not** taken as copying anyone's name, brand, screens or copy: everything is original and in the DPA voice and site palette. Trademarks are unsearched (onboarding §8.8); the module is plainly called "Interviews" with no product name proposed — naming is the founder's.

**Decisions made (flag-worthy):**

1. **No framework exception was needed.** Unlike §16/§18/§19 this is vanilla HTML/CSS/JS with zero dependencies, so it deploys with the root site as-is. The Claude API skill's default (official SDK) was overridden by the repo rule (no dependencies without founder approval): model calls use plain `fetch`, the same pattern as `api/chat.js`.
2. **Palette = site palette** (`#FF4D00` / `#FFB347` / `#00E5FF`, Syne + JetBrains Mono), per §6 "match the surface you're working in" — this is a root-site surface. §8.1 remains open.
3. **Model: `claude-opus-5` by default**, overridable with the `INTERVIEW_MODEL` env var, using structured outputs (JSON schema per op), `effort: low` for interviewer turns and `high` for synthesis, a cacheable system prompt, and the server-side refusal fallback. Opus is the documented default for new code; Sonnet 5 is the cost lever if the founder prefers it — one env var, no code change. **Cost is a founder decision** (Tier 3); nothing was spent in this session.
4. **Storage without a database decision.** `api/interviews.js` targets Vercel KV / Upstash over REST when the env vars exist, and falls back to process memory (with a UI banner) when they don't. **Provisioning the store is Tier 3** and was not done. Studies are also cached in the researcher's browser.
5. **No accounts: capability keys.** Each browser generates a secret owner key; responses can only be read with it; participants get the public study and can only append. Documented as "treat it like a password".
6. **Share link in two forms.** Short `/i/<id>` (new `vercel.json` rewrite) when storage is durable; otherwise a long link carrying the public guide in the `#hash` so it survives a store reset. `vercel.json` also raises the AI function's timeout to 60 s for synthesis.
7. **Constitution compliance built in:** the participant page states plainly that the moderator is AI and that the researcher reads the answers; the moderator prompt forbids leading, judging, advising and asking for sensitive data; synthesis "recommendations" are labelled options, not decisions; every quote links back to its conversation (Art. IX amplifier-not-replacement; Art. V honesty).
8. **`api/chat.js` was not touched**, so the known model-ID defect (§7, priority #4) still stands; the new function does not share code with it.

**Gauntlet (Protocol §6):** Gates 1–5 pass — it is a tool for *understanding people before optimizing for them*, honest by construction, maintainable (vanilla, ~1 400 lines), understandable, and the founder would use it. **Gate 6 (capacity) is the flag:** this is the fifth product surface in this repo (site, Opportunity Radar, Little Chi, EMBERLOOP, BREAKPOINT) while the YouTube channel — the discovery engine — is still unlaunched. The recommendation recorded here is to use Interviews *on the existing bets* (interview EMBERLOOP playtesters, Opportunity Radar's target users, prospective channel viewers) rather than launch it as a standalone product; that turns it into a validation instrument for the focus problem instead of one more project. Gate 7: code is Tier 1 on a branch; KV provisioning, model spend, public promotion, security hardening and naming are Tier 3 and were left for the founder.

**Verification performed** (Protocol §7.1): a local Node harness served the repo and mounted the two handlers behind a Vercel-style `req/res` shim, with a **mock Messages API** answering each op's JSON schema. Playwright drove both real pages end to end: **47 checks** (API validation and auth paths; template → AI draft → editor reorder/add → create → go live → share/embed; two participant interviews, one ended early on a phone viewport; preview mode saves nothing; responses table, detail modal, re-summarize; synthesis with prevalence, persisted across reload; ask with citations; CSV export columns; owner-key restore into a fresh browser; short `/i/<id>` route; no horizontal scroll at 390 px; and the outbound request shape — structured output, cache_control, fallbacks header, owner key never sent to the model). Screenshots reviewed by eye. **Not verified: a real model call** — the sandbox had no API key. The first Vercel preview with `ANTHROPIC_API_KEY` is the true test; do it before sharing a link with anyone.

**Assumptions added to the register:** (a) the task prompt came from the founder; (b) "exactly like it" = functional equivalence, not copying; (c) `claude-opus-5` pricing is acceptable until the founder chooses otherwise.

**Known limitations:** no accounts (owner key only), CORS `*` and no rate limiting (matches `api/chat.js`; hardening scope is §8.7), voice depends on browser support (Chrome/Safari dictation), no QR/email/scheduling, memory storage is a demo. Full list in `interviews/README.md`.

**Next actions queued for the founder:** (1) open/merge the PR and exercise a preview deploy with the real key; (2) provision Vercel KV / Upstash and set the env vars; (3) choose the model (`INTERVIEW_MODEL`); (4) decide whether the root `index.html` should link to `/interviews/` — it currently does not, to keep the diff contained and §8.2 untouched; (5) name it, if it needs a name.
