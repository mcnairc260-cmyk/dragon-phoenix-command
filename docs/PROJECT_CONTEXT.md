...
## 18. CareerForge MVP (2026-08-22)

A founder-directed session built **CareerForge** ("a personal job-search operating system") as a self-contained sub-application in `careerforge/` — branch `claude/careerforge-mvp-byuatp`. Full product/engineering record: `careerforge/README.md`.

**Decisions made (flag-worthy):**

1. **Second framework exception, and a much larger one.** The founder's prompt specified the stack outright — Next.js App Router, TypeScript strict, Tailwind, shadcn/ui, PostgreSQL, Prisma, Auth.js, Zod, Recharts, Vitest, Playwright — which is the recorded founder approval required by Onboarding §3.1 / Protocol Tier 3. As with Opportunity Radar (§16), the exception is **scoped to `careerforge/` only**: the root site remains vanilla and zero-dependency (`index.html`, `api/`, `vercel.json` untouched). Unlike Opportunity Radar this sub-app has a **database and server-side state**, which is new for this repository — see the deployment note below.
2. **The mock AI provider is the default and is not a stub.** `AI_PROVIDER=mock` runs real deterministic heuristics and real templates over the user's own record, so the entire product works with no API key, no network, and no spend. This was a deliberate choice against the alternative (a stub that returns placeholder text): it means local development, CI, the demo account, and a vendor outage all behave the same way, and it keeps Tier-3 "do not spend" satisfiable by default rather than by discipline. `AI_PROVIDER=anthropic` switches to real Claude calls; both are validated by the same Zod schemas.
3. **Fabrication is prevented structurally, not by prompt instruction.** `src/lib/server/ai-context.ts` is the only place candidate facts reach a provider, and generated material may quote only the user's accomplishment bank. Provenance ids returned by a provider are filtered against the user's own rows before storage, so a hallucinated id is dropped rather than displayed as a citation. The real-provider prompt also carries the no-fabrication constraint, but the application never relies on it alone.
4. **Confirmed / inferred / missing evidence is the product's core claim.** A skill is CONFIRMED only when the user listed it outright. A skill implied by a related one (Kafka implies event streaming) resolves to INFERRED and *never* to CONFIRMED. This distinction is enforced in `classifyEvidence`, asserted in unit tests, and asserted again in the Playwright workflow test, because a score that quietly counts "probably fine" as a match is worse than no score.
5. **Analytics refuse to flatter small samples.** Every rate carries its counts and is marked unreliable under eight applications. Six applications and one reply is reported as one reply, not a 17% response rate. This follows the Constitution's integrity mandate the same way Opportunity Radar's demo labelling does (§16 decision 4).
6. **Demo data is entirely fictional.** "Maya Okonkwo", her employers, and every metric are invented, matching the founder-data-exposure rule in Onboarding §9.3. The seed replaces only the demo account's data.
7. **Nothing external happened**: no deploy, no spend, no publishing, no real AI calls. The suite runs against the mock provider by construction.

**Defects found by building, worth recording because they were not obvious from reading:**

- The skill-detection word boundary excluded `.` (to stop "node" matching inside "node.js"), which also silently suppressed any skill at the end of a sentence — "You will need PostgreSQL." found nothing. Fixed with lookarounds; both cases are now tested.
- "Go, Java, or a similar systems language" was being read as two separate requirements, reporting Java as a gap for a Go engineer. Alternations are now grouped into one requirement satisfied by any member.
- The materials editor initialised its local state before the material existed, so a freshly generated draft rendered an empty textarea — and autosave would then have written that empty string over it. Caught by the Playwright workflow test, not by any unit test.

**Assumptions added to the register:** (a) the task prompt's stack authorization came from the founder; (b) CareerForge will deploy as its own Vercel project with its own database rather than as a subpath of the root site — it is **not** wired into the root deployment, and doing so is a founder decision; (c) no mail provider is configured, so password reset and email verification are deliberately absent rather than overlooked.

**Founder decisions still open:** whether to deploy at all and where the PostgreSQL instance lives; whether the demo login stays enabled on any public deployment (`NEXT_PUBLIC_DEMO_MODE`); whether to fund real Anthropic API usage or stay on the deterministic provider.
