...
**Merged to production (2026-08-22, founder action).** The founder merged **PR #12** into `main` (merge commit `0c03ff4`), which supersedes the "merging to `main` was not done" note earlier in this section. `main` now contains `games/emberloop/`, so the `emberloop` Vercel project builds the game from its production branch instead of from feature-branch previews: **https://emberloop.vercel.app** serves the merged build (verified — it is serving `assets/index-C83yRpT-.js`, the bundle produced by the pacing/controls commit `6558bdf`). The root `dragon-phoenix-command` project is still zero-build and still serves the site from the same `main`; the two projects have different root directories and do not interact. **Follow-up EMBERLOOP work must start from a fresh branch off `main`** — `claude/emberloop-mobile-game-qr1d70` and PR #12 are finished history and must not be reused.
