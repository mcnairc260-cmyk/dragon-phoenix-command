...
- **Genuinely verified:** 93 C# tests passing under a standalone Mono harness — parity, physics behaviour, determinism, shot records, render-frame conversion, generated table geometry, one full demonstration shot, and allocation. This is real, and it covers all the authoritative code.
...
**The allocation audit found and fixed a real hot-loop allocation.** The simultaneous-contact solver cloned a `BallBody` per body per relaxation pass, costing about **85 KB of garbage per break**. Pooling those scratch bodies removed it: a warmed-up world now allocates **zero bytes** across a whole shot, asserted by a test rather than merely observed, and a cold break costs ~10 KB (all event-list growth). The parity fixtures pass bit-identically either side of the change, which is what proves the physics did not move. There is no LINQ anywhere in the simulation. Throughput measured at ~8 µs per 120 Hz step, ~1000× realtime — on this desktop runtime, with no rendering, and **not** on a phone.

**A single demonstration shot is verified end to end in the simulation**: a straight pot into the top-right corner with right-hand English produces a strike, a ball-ball contact, a cushion rebound, a pocket capture and a complete settle in one shot — and the *only* difference between the right-English, left-English and draw runs is where the tip met the ball, yet the cue ball finishes more than 100 mm apart each time. That is the "do not fake spin visually" requirement stated as an experiment. The same shot is written as a play-mode test through the Unity bridge; that version has never run.
...
**Phase 2 was not started.** No WPA rules, no turn or foul logic, no AI, no progression, no cosmetics system, no multiplayer, no monetisation.

**BREAKPOINT has moved to its own repository.** `mcnairc260-cmyk/BREAKPOINT` is now the canonical home and the source of truth: `unity/` (primary implementation) and `reference/threejs-phase1/` (the physics oracle, preserved intact), with both CI workflows. PR #2 there. **The copies under `games/breakpoint/` and `games/breakpoint-unity/` in this monorepo are historical** — develop BREAKPOINT in the standalone repository, not here. Nothing was deleted from this repo; the branches remain as the record of how the work was built. `BreakpointTheme` is the seam a cosmetics system would attach to and nothing more. Nothing was published, deployed, or paid for.
