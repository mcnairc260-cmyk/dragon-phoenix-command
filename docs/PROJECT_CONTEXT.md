...
**Page 5's border (2026-08-05).** Every page seats its panel at **(38,37)-(1668,1669)** — a uniform ~37px white margin. The founder's replacement page 5 was the correct width but only **1304px tall**, leaving ~200px white bands top and bottom, so it read as a different trim size from the rest of the book. Scaling the panel to fill would have cropped 203px from each side and **cut the master's balloon in half**, so instead the two bands of plain background — ceiling above, floor below the lowest balloon — were stretched 2.78× to make up the 329px. Nothing is cropped and the middle of the panel is untouched: characters, table, bonsai and all three balloons keep their original scale and relative positions, and the seams where the stretch ends are invisible because both bands are flat receding planes. Page 5's own frame bars are thinner than the original pages' ~10px frame; that is the founder's artwork and was carried over rather than redrawn. **Page 6 had a milder case of the same defect** (block 1620×1542, margins 48/79/39/86) and was corrected 2026-08-05 by a different method. Page 5's technique does not transfer, because page 6 is a two-panel page with **no full-width band of plain background to stretch** — shelves and a window run to the top of the upper panel, the table and figures to the bottom of the lower one. Its block was instead scaled **uniformly** to the box's height and the 85px of excess width cropped, all of it from the left: the elder's balloon reaches to within 16px of the right border, so a centred crop clips it, while the left edge carries only a bench edge and a background monk's shoulder. Scaling uniformly rather than squeezing to fit was deliberate — filling the box anamorphically needs a 5.2% vertical stretch, which would make every face on the page taller.

**The rule both pages establish:** the panel box is **(38,37)-(1668,1669)**. Check it on any new or replaced page. Fill it by stretching plain background where the page has some, and by uniform-scale-and-crop where it does not — never by anamorphic scaling, and never with a crop that reaches a balloon.
...
- A tail's mouth must be cut at the radius where the outline is actually **drawn**, not at r=1. `draw_tail` measures it per balloon (`_outline_radius`) and fills from r=0.94 out past it to the tip; filling only to r=1.03 leaves the outline arc intact and it reads as **a line ruled across the back of the tail**. The strokes start at the measured radius so they merge with the outline instead of ending short of it.
- A cloned background patch inherits whatever was in its source. Page 6's tail was covered by cloning the wall from 150px left, and that source clips the balloon outline, so the clone dropped fragments of black onto the wall. `despeckle` clears isolated fragments inside the cloned box; it is deliberately limited to small components well clear of the balloon, because near the outline the leftover ink and the real linework touch and cannot be told apart automatically.
